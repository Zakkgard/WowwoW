...
{
	public class Online : IInitialize
	{
		public static void Initialize()
		{
			Commands.Register( "Online", AccessLevels.Admin, new CommandEventHandler( Global_OnCommand ) );
		}
		...
		private static bool Global_OnCommand( CommandEventArgs e )
		{
			...
			//MobileList.MobileEnumerator enumerator = World.allConnectedChars.GetEnumerator();
			try
			{
				foreach( Mobile targ in World.allConnectedChars )
				{
					if ( prop.Check( targ ) )
					{
			...
			e.SendMessage(String.Format( "Command correctly used on {0} of {1} online Characters in the world", CorrectCount, AllCount ));
			return true;
