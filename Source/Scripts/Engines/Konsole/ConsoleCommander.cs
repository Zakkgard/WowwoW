...
namespace Server.ConsoleCommander 
{
	...
	{ 
		...
		/// <summary>
		/// Execute only in Main thread ( by WowTimer )
		/// </summary>
		public virtual bool ExecInMain
		{
			get { return true; }
		}
	} 
	...
	public sealed class ConsoleCommander : WowTimer, IInitialize 
	{
		#region Private work class 
		private sealed class Work
		{
			private string[] _param;
			public string[] Param { get { return _param; } }
			private BaseCommand _cmd;
			public BaseCommand Cmd { get { return _cmd; } }
			public Work( string[] param, BaseCommand cmd )
			{
				_param = param;
				_cmd = cmd;
			}
		...
		#region Main Thread ( WowTimer ) 

		private static ConsoleCommander.Work _job = null;
		private static ConsoleCommander _syncTool = new ConsoleCommander();

		public ConsoleCommander(): base( WowTimer.Priorities.Second5, 5 )
		{
		}
		...
		#region Console Thread ( Console Commander ) 
		...
		/// <summary>
		/// Sleep before new command
		/// </summary>
		private static TimeSpan sleep = new TimeSpan( 0, 0, 1 );
		...
		/// <summary>
		/// Add new command into pool
		/// </summary>
		public static void AddCommand( BaseCommand cmd ) 
		{ 
			foreach ( string cmdName in cmd.Commands )
			{
				string _cmdname = cmdName.ToLower();
				if ( !commandList.ContainsKey( _cmdname ) ) 
				{
					commandList.Add( _cmdname, cmd ); 
				}
			}
		} 

		/// <summary>
		/// static Initialisation events
		/// </summary>
		public static void Initialize() 
		{ 
			Console.WriteLine( "Console Commander addon v2.4" );
			_syncTool.Start();
		...
		/// <summary>
		/// Internal Console Thread loop
		/// </summary>
		private static void ConsoleThread( object state ) 
		{ 
			Thread.CurrentThread.Priority = ThreadPriority.Lowest;
			while ( World.Loading ) { Thread.Sleep( sleep ); }
...
namespace Server.ConsoleCommander 
{
	...
	{ 
		...
		{ 
			try 
			{
				...
				Console.WriteLine("> Physical Memory:\t{0}",         p.WorkingSet64 / 1024 + "k"); 
				Console.WriteLine("> Peak Physical Memory:\t{0}",      p.PeakWorkingSet64 / 1024 + "k"); 
				Console.WriteLine("> Virtual Memory:\t{0}",            p.VirtualMemorySize64 / 1024 + "k"); 
				Console.WriteLine("> Peak Virtual Memory:\t{0}",      p.PeakVirtualMemorySize64 / 1024 + "k"); 
				Console.WriteLine("> NonPaged Memory:\t{0}",         p.NonpagedSystemMemorySize64 / 1024 + "k"); 
				Console.WriteLine("> Paged Memory:\t\t{0}",            p.PagedMemorySize64 / 1024 + "k"); 
				Console.WriteLine("> Private Memory: \t{0}",         p.PrivateMemorySize64 / 1024 + "k"); 
	...
	#region ErrorListGen Command 
	public class ErrorListGenCommand : BaseCommand, IInitialize 
	{ 
		public static void Initialize() 
		{ 
		...
		public ErrorListGenCommand (): base( new string[] { "ErrorListGen", "ELG" }, "check files: QWarnings.txt, NotExist.txt", "generate list of quest errors and not present items/mobiles/gameObjects", "ELG" ) 
		{}

		protected override void Event( params string[] list ) 
		{
			Console.WriteLine( "Check init QuestCheck engine " );
			NPCQuests.Init();
			Console.WriteLine( "Start write QWarnings.txt ..." );
			NPCQuests.LOGErrors( "QWarnings.txt", false );
			Console.WriteLine( "Start write NotExist.txt ..." );
			BadIdList.GenerateLog( "NotExist.txt", false );
